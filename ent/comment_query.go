...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withVideo; query != nil {
		if err := cq.loadVideo(ctx, query, nodes, nil,
			func(n *Comment, e *Video) { n.Edges.Video = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadVideo(ctx context.Context, query *VideoQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Video)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Comment)
	for i := range nodes {
		if nodes[i].video_comments == nil {
			continue
		}
		fk := *nodes[i].video_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "video_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
