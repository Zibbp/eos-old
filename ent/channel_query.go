...
func (cq *ChannelQuery) QueryVideos() *VideoQuery {
...
func (cq *ChannelQuery) IDs(ctx context.Context) ([]string, error) {
	var ids []string
...
func (cq *ChannelQuery) Clone() *ChannelQuery {
	...
	return &ChannelQuery{
		...
		withVideos: cq.withVideos.Clone(),
...
func (cq *ChannelQuery) WithVideos(opts ...func(*VideoQuery)) *ChannelQuery {
	query := (&VideoClient{config: cq.config}).Query()
...
func (cq *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := cq.withVideos; query != nil {
		if err := cq.loadVideos(ctx, query, nodes,
			func(n *Channel) { n.Edges.Videos = []*Video{} },
			func(n *Channel, e *Video) { n.Edges.Videos = append(n.Edges.Videos, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChannelQuery) loadVideos(ctx context.Context, query *VideoQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Video)) error {
	...
	query.Where(predicate.Video(func(s *sql.Selector) {
		s.Where(sql.InValues(channel.VideosColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.channel_videos
		if fk == nil {
			return fmt.Errorf(`foreign-key "channel_videos" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "channel_videos" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
