...
func (*Playback) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case playback.FieldVideoID, playback.FieldStatus:
			values[i] = new(sql.NullString)
		case playback.FieldCreatedAt, playback.FieldUpdatedAt:
	...
	for i := range columns {
		...
		case playback.FieldVideoID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				pl.VideoID = value.String
			}
		case playback.FieldTimestamp:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field timestamp", values[i])
			} else if value.Valid {
				pl.Timestamp = int(value.Int64)
			}
...
func (pl *Playback) String() string {
	...
	builder.WriteString(pl.VideoID)
	builder.WriteString(", ")
	builder.WriteString("timestamp=")
	builder.WriteString(fmt.Sprintf("%v", pl.Timestamp))
	builder.WriteString(", ")
